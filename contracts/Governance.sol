  ...
  RoleVoting
{
  ...
  bytes32 private constant _UNDELEGATION_TYPEHASH =
    keccak256('Undelegation(address delegatee,uint256 nonce,uint256 expiry)');
  ...
  function delegateBySig(
    ...
    address signer = ECDSA.recover(
      _hashTypedDataV4(
        keccak256(
          abi.encode(_DELEGATION_TYPEHASH, role, delegatee, nonce, expiry)
        )
      ),
      v,
      r,
  ...
  function undelegateBySig(
    ...
    address signer = ECDSA.recover(
      _hashTypedDataV4(
        keccak256(
          abi.encode(_UNDELEGATION_TYPEHASH, role, delegatee, nonce, expiry)
        )
      ),
      v,
      r,
  ...
  function _countVote(
    ...
    for (uint256 i = 0; i < proposal.roles.length; ++i) {
      if (
        (proposal.roles.length == 1 && proposal.roles[0] == EMPTY_ROLE) ||
